                ...
                if (Array.isArray(updated[field])) {
                  if (typeof value === "object" && value !== null && "$in" in value) {
                    updated[field] = updated[field].filter(
                      (item: any) => !(value as any).$in.includes(item)
                    );
                  } else {
  ...
  describe("Query Integration", () => {
    ...
    it("should perform complex queries with FlongoQuery", async () => {
      ...
      const activeAdults = await collection.getAll(
        new FlongoQuery().where("isActive").eq(true).and("age").gt(25)
      );
    ...
    it("should handle pagination with queries", async () => {
      const firstPage = await collection.getAll(new FlongoQuery().where("isActive").eq(true), {
        offset: 0,
        count: 1
      });
      ...
      const secondPage = await collection.getAll(new FlongoQuery().where("isActive").eq(true), {
        offset: 1,
        count: 1
      });
    ...
    it("should handle array queries", async () => {
      const developers = await collection.getAll(
        new FlongoQuery().where("tags").arrContainsAny(["developer"])
      );
    ...
    it("should handle count and exists operations", async () => {
      const activeCount = await collection.count(new FlongoQuery().where("isActive").eq(true));
      expect(activeCount).toBe(2);

      const hasAdults = await collection.exists(new FlongoQuery().where("age").gtEq(18));
