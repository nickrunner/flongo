...
describe('InvalidationStrategy', () => {
  ...
  describe('invalidateOnBulkUpdate', () => {
    it('should invalidate entire collection when no query specified', async () => {
      const key1 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id: '1' });
      const key2 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'getAll' });
      const key3 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'count' });
    ...
    it('should invalidate overlapping queries when query specified', async () => {
      ...
      const key1 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'getAll', query: query.build() });
      const key2 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'count' });
  ...
  describe('invalidateOnBulkDelete', () => {
    it('should invalidate all specified documents and collection queries', async () => {
      const ids = ['1', '2', '3'];
      const key1 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id: '1' });
      const key2 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id: '2' });
      const key3 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'getAll' });
      const key4 = CacheKeyGenerator.generate({ collection: collectionName, operation: 'count' });
