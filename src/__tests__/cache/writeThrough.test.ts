...
describe('WriteThrough', () => {
  ...
  describe('handleCreate', () => {
    it('should cache newly created document', async () => {
      ...
      const cacheKey = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id: document._id });
  ...
  describe('handleUpdate', () => {
    it('should update cached document', async () => {
      ...
      const cacheKey = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id });
      const cached = await cacheStore.get(cacheKey);
  ...
  describe('handleBatchUpdate', () => {
    it('should update multiple documents in cache', async () => {
      ...
      for (const doc of documents) {
        const cacheKey = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id: doc._id });
        const cached = await cacheStore.get(cacheKey);
  ...
  describe('Invalidation on atomic operations', () => {
    beforeEach(async () => {
      ...
      const cacheKey = CacheKeyGenerator.generate({ collection: collectionName, operation: 'get', id: '123' });
      await cacheStore.set(cacheKey, document);
